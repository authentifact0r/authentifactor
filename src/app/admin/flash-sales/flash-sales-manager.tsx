    ...
    <div className="p-6 md:p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-white/60 mt-1">{activeSales.length} active · {expiredSales.length} expired</p>
        </div>
      ...
      {showCreate && (
        <div className="rounded-2xl border border-amber-500/20 bg-amber-500/[0.03] p-6 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              <label className="block text-xs font-medium text-white/70 mb-1">Duration (hours)</label>
            ...
            <div>
              <label className="block text-xs font-medium text-white/70 mb-1">Reason / Label</label>
      ...
      {activeSales.length > 0 && (
        <div className="space-y-2">
          <h2 className="text-xs font-semibold uppercase tracking-wider text-white/60">Active Sales</h2>
              ...
              <div key={sale.id} className="rounded-2xl border border-amber-500/20 bg-amber-500/[0.03] overflow-hidden">
                <button onClick={() => setExpanded(isOpen ? null : sale.id)} className="w-full flex items-center gap-4 px-5 py-4 hover:bg-white/[0.02] transition-colors text-left">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-white/50">{sale.reason || "Flash Sale"}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm text-white/60 line-through">{formatPrice(sale.product.price)}</p>
                  ...
                  <span className="hidden sm:flex items-center gap-1 text-xs text-white/60"><Clock className="h-3 w-3" /> {remaining}</span>
                ...
                {isOpen && (
                  <div className="border-t border-white/[0.04] bg-white/[0.01] p-5 space-y-3">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                      <div><span className="text-xs text-white/60">Original</span><p className="text-white font-medium">{formatPrice(sale.product.price)}</p></div>
                      <div><span className="text-xs text-white/60">Sale Price</span><p className="text-amber-400 font-bold">{formatPrice(salePrice)}</p></div>
                      <div><span className="text-xs text-white/60">Started</span><p className="text-white/60">{new Date(sale.startsAt).toLocaleString("en-GB")}</p></div>
                      <div><span className="text-xs text-white/60">Ends</span><p className="text-white/60">{new Date(sale.endsAt).toLocaleString("en-GB")}</p></div>
                    </div>
      ...
      {expiredSales.length > 0 && (
        <div className="space-y-2">
          <h2 className="text-xs font-semibold uppercase tracking-wider text-white/50">Expired / Paused</h2>
          {expiredSales.map(sale => (
            <div key={sale.id} className="flex items-center gap-4 rounded-2xl border border-white/[0.06] bg-white/[0.02] px-5 py-3 opacity-60">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-white/50">{sale.discountPercent}% off · {sale.reason || "Flash Sale"}</p>
              </div>
              <button onClick={() => deleteSale(sale.id)} className="text-xs text-white/50 hover:text-red-400 transition">Remove</button>
            </div>
          ))}
        </div>
      )}
      ...
      {sales.length === 0 && !showCreate && (
        <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-12 text-center">
          ...
          <p className="text-white/60">No flash sales yet.</p>
          <p className="text-xs text-white/50 mt-1">Create a time-limited discount to boost sales.</p>
        </div>
      )}
    </div>
  );
